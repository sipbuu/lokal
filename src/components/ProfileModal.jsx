  ...
  const handleAvatarFile = async (file) => {
    ...
    reader.onload = () => {
      ...
      image.onload = async () => {
        ...
        ctx.drawImage(image, offsetX, offsetY, size, size, 0, 0, 300, 300)
        const avatarData = canvas.toDataURL('image/jpeg', 0.85)
        const updated = await api.updateProfile({ userId: user.id, avatarData })
        persistUser(updated ? { ...updated, avatar_updated_at: Date.now() } : null)
        setUploading(false)
      }
      image.onerror = () => setUploading(false)
      image.src = src
    }
